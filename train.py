...
class Trainer:
    ...
    def __init__(self, model, model_old, device, opts, trainer_state=None, classes=None, step=0):
        ...
        self.feature_matching_loss_flag = opts.feature_matching
        self.logit_matching_loss_flag = opts.logit_matching
        ...
        self.colormap = opts.dataset

        self.weighted_ce = opts.weighted_ce

        self.is_separate = opts.method == "SEPARATE" or opts.separate
    ...
    def before(self, train_loader, logger):
        ...
        if self.weighted_ce and self.step > 0:
            self.find_class_weight(train_loader, self.device, logger)
        

    def separate_train(self, cur_epoch, optim, train_loader, scheduler=None, print_int=10, logger=None):
        ...
        loss_feature_match = torch.tensor(0.)
        loss_logit_match = torch.tensor(0.)
        ...
        model.train()
        logger.info('Separate Training')

        if self.feature_matching_loss_flag:
            feature_matching_loss = nn.MSELoss()
        if self.logit_matching_loss_flag:
            logit_matching_loss = nn.MSELoss(reduction='none')
        ...
        for cur_step, (images, labels) in enumerate(train_loader):
            ...
            label2color = utils.Label2Color(cmap=utils.color_map(self.colormap))  # convert labels to images
            ...
            local_kernel = torch.ones(self.old_classes, 1, 3, 3)
            local_kernel = local_kernel.to(device, dtype=torch.float32)
            local_kernel[:, :, 1, 1] = 0.0
            local_kernel = local_kernel / 8.0
            ...
            if self.step > 0:
                ...
                if self.pseudo_labeling == "naive":
                    ...
                    local_outputs = F.conv2d(outputs_old, local_kernel, None, 1, 1, groups=self.old_classes )
                    local_probs = torch.softmax(local_outputs, dim=1)
                    max_probs_local, pseudo_labels_local = local_probs.max(dim=1)
                    mask_valid_pseudo_local = (torch.eq(pseudo_labels, pseudo_labels_local)
                                        # & (entropy(local_probs) < entropy(probs))
                                        )
                    ...
                    if self.classif_adaptive_factor:
                        ...
                        classif_adaptive_factor = torch.cat([classif_adaptive_factor, classif_adaptive_factor], 0)   
                                            
                                         
            norm = transform.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])
            labels_full = labels.clone()
              
            labels[~mask_background] = 255
            
            mask_background = mask_background.unsqueeze_(1)
            mask_background = torch.cat([mask_background, mask_background, mask_background], dim=1)
            
            for i in range(images.shape[0]):
                #im = images[i, :, :, :].cpu().numpy()
                im_copy = images_copy[i, :, :, :].cpu().numpy()
                
                mask = mask_background[i, :, :, :].cpu().numpy()
                
                #im = denorm(im)
                im_copy = denorm(im_copy)
                
                im_copy[~mask] = 0
                
                im_copy = norm(torch.from_numpy(im_copy).cuda())
                
                images_copy[i, :, :, :] = im_copy
            
            
            
            images = torch.cat([images_original, images_copy], 0)
            labels = torch.cat([labels_full, labels], 0)
            ...
            if self.pseudo_soft is not None:
                ...
                elif self.weighted_ce and self.step > 1000:
                    # weight_background, weight_old, weight_new = self.calculate_ratio(labels)
                    weight = torch.ones(outputs.shape[1]).cuda()
                    weight[0] = self.weight_background
                    weight[1:self.old_classes] = self.weight_old
                    weight[self.old_classes:] = self.weight_new
                    # print(weight)
                    criterion = nn.CrossEntropyLoss(ignore_index=255, weight=weight, reduction='none')
            ...
            '''
            if self.weighted_ce:
                sample_weights = self.calculate_sample_weight(labels_full)
                if self.is_separate:
                    sample_weights = torch.cat([sample_weights, torch.ones(sample_weights.size()).to(device)], 0)
            '''
            '''
            ...
            '''
            if self.weighted_ce:
                pre_sample_weights = self.calculate_sample_weight(labels_full)
                if self.is_separate:
                    pre_sample_weights = torch.cat([pre_sample_weights, torch.ones(pre_sample_weights.size()).to(device)], 0)
                sample_weights = torch.ones(loss.size()).to(device)
                for i in range(pre_sample_weights.size(0)):
                    sample_weights[i][(labels[i] > 0) & (labels[i] < self.old_classes)] = pre_sample_weights[i]
            if sample_weights is not None:
                # loss = loss * sample_weights.unsqueeze(-1).unsqueeze(-1)
            ...
            if self.feature_matching_loss_flag and self.step > 0:
                mask_old = labels_full < self.old_classes
                mask_old = mask_old * labels_full != 0
                feature_map = features["pre_logits"]

                feature_map = feature_map.clone()
                feature_map = torch.abs(feature_map)
                feature_map = torch.sum(feature_map, dim=1, keepdim=True)
                feature_map = F.interpolate(
                    feature_map, size=images.shape[-2:], mode="bilinear", align_corners=False
                )
                feature_map = feature_map.squeeze()

                batch = int(feature_map.shape[0] / 2)
                feature_map_first = feature_map[:batch,:,:]
                feature_map_second = feature_map[batch:, :, :]

                feature_map_first = feature_map_first * mask_old
                feature_map_second = feature_map_second * mask_old

                x = feature_map_first.unsqueeze_(1)

                x = feature_map_second.unsqueeze_(1)

                loss_feature_match = 0.01 * feature_matching_loss(feature_map_first, feature_map_second)
            
            if self.logit_matching_loss_flag and self.step > 0:

                mask_old = ((labels < self.old_classes) * labels) != 0
                old_sum = torch.sum(mask_old)
                if old_sum == 0: old_sum = 1
                loss_logit_match = logit_matching_loss(torch.reshape(outputs[:, :self.old_classes, :, :].permute(0,2,3,1),(-1, self.old_classes)),
                                                    torch.reshape(torch.cat([outputs_old[:outputs_old.size(0)//2], outputs_old[outputs_old.size(0)//2:]]).permute(0,2,3,1),(-1, self.old_classes)))
                loss_logit_match = 0.01 * (torch.nansum(mask_old.view(-1).unsqueeze(-1) * loss_logit_match) / old_sum)

            # xxx first backprop of previous loss (compute the gradients for regularization methods)
            loss_tot = loss + lkd + lde + l_icarl + pod_loss + loss_entmin + loss_feature_match + loss_logit_match
            ...
            ) + loss_entmin.item() + loss_feature_match.item()
            ...
            if (cur_step + 1) % print_int == 0:
                ...
                logger.info(
                    f"Loss made of: CE {loss}, LKD {lkd}, LDE {lde}, LReg {l_reg}, POD {pod_loss} EntMin {loss_entmin} FM {loss_feature_match} LM {loss_logit_match}"
                )
                # visualization
    ...
    def train(self, cur_epoch, optim, train_loader, scheduler=None, print_int=10, logger=None):
        ...
        criterion = self.criterion
        if self.weighted_ce:
            criterion = nn.CrossEntropyLoss(ignore_index=255, weight=self.weight, reduction='none')
        ...
        for cur_step, (images, labels) in enumerate(train_loader):
            ...
            if self.step > 0:
                ...
                if self.pseudo_labeling == "naive":
                    ...
                    pseudo_labels[max_probs < self.thresholds[pseudo_labels]] = 255
                    labels[mask_background] = pseudo_labels[mask_background]
                elif self.pseudo_labeling == "entropy" or self.pseudo_labeling == "multi-entropy":
                    probs = torch.softmax(outputs_old, dim=1)
                    max_probs, pseudo_labels = probs.max(dim=1)
                    ...
                    mask_valid_pseudo_local = (torch.eq(pseudo_labels, pseudo_labels_local)
                                        # & (entropy(local_probs) < entropy(probs))
                                         & (entropy(local_probs) < self.thresholds[pseudo_labels] * 1.2)
                                        )

                    if self.pseudo_labeling == "multi-entropy":
            ...
            if self.pseudo_soft is not None:
                ...
                if self.ce_on_pseudo and self.step > 0:
                    ...
                    _labels[_labels == 0] = 255
                    loss = criterion(outputs, _labels)  # B x H x W
                elif self.weighted_ce and self.step > 0:
                    loss = criterion(outputs, labels)
                else:
                    loss = criterion(outputs, labels)  # B x H x W
    ...
    def find_median(self, train_loader, device, logger, mode="probability"):
        ...
        """
        if mode == "entropy" or mode == "multi-entropy": ## modified: add "multi-entropy" strategy
        ...
        for cur_step, (images, labels) in enumerate(train_loader):
            ...
            if mode == "multi-entropy":
                values_to_bins = entropy(probas)[mask_bg].view(-1) / max_value
                label_list = [pseudo_labels]
                for t in range(2):
                    images_tmp = images + torch.randn(images.size()) * 0.06
                    outputs_tmp, _ = self.model_old(images_tmp, ret_intermediate=False)

                    probas_tmp = torch.softmax(outputs_tmp, dim=1)
                    _, pseudo_labels_tmp = probas_tmp.max(dim=1)
                    values_to_bins = torch.cat([values_to_bins, entropy(probas_tmp)[mask_bg].view(-1) / max_value])
                    label_list.append(pseudo_labels_tmp)
            ...
            x_coords = torch.cat([lbl[mask_bg].view(-1) for lbl in label_list]) if mode == "multi-entropy" else pseudo_labels[mask_bg].view(-1)
        ...
        with torch.no_grad():
            for i, (images, labels) in enumerate(loader):
                ...
                if ret_samples_ids is not None and i in ret_samples_ids:  # get samples
                    ret_samples.append((denorm(images[0]).detach().cpu().numpy(), labels[0], prediction[0]))
    ...
    def calculate_ratio(self, labels):
        background = labels == 0
        old_pixel = labels < self.old_classes
        old_pixel = old_pixel & (~background)
        new_pixel = labels >= self.old_classes
        new_pixel = new_pixel & (labels != 255)
        background = torch.sum(background)
        old_pixel = torch.sum(old_pixel)
        new_pixel = torch.sum(new_pixel)

        weight_background = new_pixel / background
        weight_old = new_pixel / old_pixel
        weight_new = new_pixel / new_pixel
        sum = weight_background + weight_old + weight_new
        weight_background = weight_background / sum
        weight_old = weight_old / sum
        weight_new = weight_new / sum
        return weight_background, weight_old, weight_new
    ...
    def calculate_sample_weight(self, labels):
        ...
        new_pixel = labels >= self.old_classes
        new_pixel = new_pixel & (labels != 255)
        old_pixel = torch.sum(old_pixel, (1,2))
        new_pixel = torch.sum(new_pixel, (1,2))
        new_pixel[new_pixel==0] = 1
        weight = 0.5 + F.sigmoid(old_pixel/new_pixel)
        return weight
    
    def find_class_weight(self, train_loader, device, logger):

        background = torch.tensor(0)
        old_pixel = torch.tensor(0)
        new_pixel = torch.tensor(0)
            ...
            if self.step > 0:
                ...
                if self.pseudo_labeling == "naive":
                    ...
                    if self.pseudo_soft is None:
                        ...
                        labels[mask_valid_pseudo & mask_background] = pseudo_labels[mask_valid_pseudo &
                                                                                    mask_background]
                background_pixels = labels == 0
                background = background + torch.sum(background_pixels)
                old_pixel = old_pixel + torch.sum((labels < self.old_classes) & (~background_pixels))
                new_pixel = new_pixel + torch.sum((labels >= self.old_classes) & (labels != 255))
        
        with torch.no_grad():
            
            self.weight_old = 0.5 + torch.sigmoid((new_pixel / self.nb_new_classes - (old_pixel + background) / (self.old_classes) ) / ((old_pixel + background) / (self.old_classes)))
            self.weight_new = 1.
            self.weight_background = 1.
            weight = torch.ones(self.nb_current_classes).cuda()
            weight[0] = self.weight_old
            weight[1:self.old_classes] = self.weight_old
            weight[self.old_classes:] = self.weight_new
            self.weight = weight
            print(self.weight_background, self.weight_old, self.weight_new)
