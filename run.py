...
def run_step(opts, world_size, rank, device):
    ...
    if TRAIN:
        ...
        for cur_epoch in range(opts.epochs):
            ...
            if opts.separate and opts.step > 0:
              epoch_loss = trainer.separate_train(
                  cur_epoch=cur_epoch,
                  optim=optimizer,
    ...
    val_loss, val_score, ret_samples = trainer.validate(
        loader=test_loader, metrics=val_metrics, logger=logger, end_task=True, ret_samples_ids=list(range(100))
    )
